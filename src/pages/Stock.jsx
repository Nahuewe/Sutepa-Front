...
export const Stock = ({ title = "Listado de Stock" }) => {
  ...
  const data = useMemo(() => stockSucursal, [stockSucursal]);
  ...
  const onPuntoGiroClick = (id) => {
    setStockSucursal(stockMaterial.find((sucursal) => sucursal.id === id)?.materiales || []);
    setActiveButton(id);
  };
  ...
  return (
    <>
      <Card>
        ...
        <div className="md:flex justify-evenly items-center p-0 mb-3">
          {
            // Creo un boton por cada sucursal con excepcion de Giro central
            sucursales.map((sucursal) => (
              (sucursal.id !== 1) && (
                <button
                  key={sucursal.id}
                  className={`btn-giro btn${activeButton === sucursal.id ? ' active' : ''}`}
                  onClick={() => onPuntoGiroClick(sucursal.id)}
                >
                  {sucursal.nombre}
                </button>
              )
